...
class Type{
    ...
    // Exemple :
    // Type.getEspece(97) doit retourner "Hypnomade"
    ...
    public static double getEfficacite(int typeAtt, int typeDef){
	    if( (typeAtt >= 0 && typeAtt < efficacite.length) && (typeDef >= 0 && typeDef < efficacite.length) ) {
    ...
    public static int getIndiceType(String type){
        for(int next = 0; next < nomsType.length; next++) {
            if(nomsType[next].equalsIgnoreCase(type)) {
                return next;
            }
        }
        return -1;
